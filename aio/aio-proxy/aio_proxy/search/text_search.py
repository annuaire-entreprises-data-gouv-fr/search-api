...
def text_search(index, search_params):
    ...
    etablissement_filter_used = is_any_etablissement_filter_used(search_params)
    ...
    if etablissement_filter_used:
        ...
        filters_etablissements_query_without_inner_hits = (
            build_nested_etablissements_filters_query(
                search_params, with_inner_hits=False
            )
        )
        ...
        if query_terms:
            text_query = build_text_query(
                terms=query_terms, matching_size=search_params.matching_size
            )
            text_query_with_filters = add_nested_etablissements_filters_to_text_query(
                text_query, search_params
            )
        ...
        # Filters applied on établissements without text search
            ...
            filters_etablissements_query_with_inner_hits = (
                build_nested_etablissements_filters_query(
                    search_params, with_inner_hits=True
                )
            )
    ...
    is_bilan_bilan_used = is_any_bilan_filter_used(search_params)
    ...
    # Search 'élus' only
    type_personne = search_params.type_personne
