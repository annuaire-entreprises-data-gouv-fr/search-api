...
@pytest.mark.parametrize("activite_principale, expected", [("62.01Z", "62.01Z"),
                                                           ("27.33Z", "27.33Z") ,
                                                           (None, None)])
def test_parse_and_validate_activite_principale(activite_principale, expected):
    assert parameters.parse_and_validate_activite_principale(activite_principale) == \
           expected
...
def test_parse_and_validate_activite_principale_fail_1(activite_principale: str):
    with pytest.raises(ValueError, match="Activité principale doit contenir 6 "
                                         "caractères."):
