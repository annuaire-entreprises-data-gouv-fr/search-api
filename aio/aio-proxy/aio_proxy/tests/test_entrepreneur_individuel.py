...
@pytest.mark.parametrize("is_entrepreneur_individuel, expected",
                         [("YES", True),
                          ("NO", False),
                          (None, None)])
def test_parse_and_validate_is_entrepreneur_individuel(
        is_entrepreneur_individuel: str, expected: str):
    assert parameters.parse_and_validate_is_entrepreneur_individuel(
        is_entrepreneur_individuel) == expected
...
def test_parse_and_validate_is_entrepreneur_individuel_fail(
    ...
    with pytest.raises(ValueError, match="Seuls les valeurs 'yes' ou bien 'no' sont "
                                         "possibles pour "
